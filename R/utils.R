...
#' @import ForecastComb
#'
#' @return Results of the forecast combination 
...
#' @title Rolling weights of Forecast Combination for Dynamic Gompertz and 
...
#' combination of forecasts. Example: \code{comb_OLS}.
#' 
#' @param plot.graph A logical value. If TRUE, computes the forecast combination 
#' dynamically, showing a graph of the evolution of weights across \code{test_days}. 
#' If FALSE, returns the rolling_comb object. Defaults to TRUE.
#' 
#' @param LeadIndCol An integer indicating the column number in \code{Y} that 
#' contains the leading indicator. Defaults to 1.
#' 
...
#' @import ForecastComb
#'
#' @return If \code{plot.graph = FALSE}, returns a ForecastComb object with 
#' details of the computed weights.
#' 
#' If \code{plot.graph = TRUE}, returns a `ggplot` object showing the evolution 
...
#'
#' @export
plot_rolling_weights=function(Y,est.start.date,est.end.date,
                           all_lags,train_days,test_days,method,
                           plot.graph=TRUE,LeadIndCol=1){  
  ...
  if (!is.numeric(all_lags) || any(all_lags < 0)) stop("all_lags must be a numeric vector with non-negative values.")
  if (!is.function(method)) stop("method must be a valid function from the ForecastComb package.")
  if (!is.logical(plot.graph)) stop("rolling must be a logical value.")
  if (!is.numeric(LeadIndCol) || LeadIndCol <= 0) stop("LeadIndCol must be a positive integer.")
